...
class OrdersBloc extends BlocBase {
  ...
  void _addOrdersListener() {
    _firestore.collection("orders").snapshots().listen((data) {
  ...
  void regredir(DocumentSnapshot order) {
    order.reference.updateData({"status": order.data["status"] - 1});
  }

  void avancar(DocumentSnapshot order) {
    order.reference.updateData({"status": order.data["status"] + 1});
  }

  void remover(DocumentSnapshot order) {
    Firestore.instance
        .collection("users")
  ...
  void sortCriteria(SortCriteria criteria) {
    _criteria = criteria;
    _sort();
